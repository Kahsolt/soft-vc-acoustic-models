...
class MelDataset(Dataset):
    ...
    def __init__(self, root: Path, train: bool = True, discrete: bool = False, split_ratio: float = 0.1):
        self.discrete = discrete
        self.mels_dir  = root / "mels"
        self.units_dir = root / "units"

        fps = [fp for fp in self.mels_dir.rglob("**/*.npy")]
        np.random.shuffle(fps)
        cp = int(len(fps) * split_ratio)
        if train: fps = fps[cp:]
        else:     fps = fps[:cp]
    ...
    def __getitem__(self, index):
        ...
        if self.discrete: units = units.long()
        return mel, units
    ...
    def pad_collate(self, batch):
        ...
        units = pad_sequence(units, batch_first=True, padding_value=100 if self.discrete else 0)
